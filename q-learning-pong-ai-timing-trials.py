...
class PongGame():#pyglet.window.Window):
    ...
    def __init__(self):
        ...
        self.times = []            # x values of trials vs. time graph.
    ...
    def plot_num_trials_verses_time(self):
        fig = plt.figure()
        plt.plot(self.times, self.sim_sample_nums)
        plt.title("Number of Trials vs. Time")
        plt.xlabel('Time (in hours)')
        plt.ylabel('Number of Trials')
        #plt.show()  # use this to display the graph.
        plt.savefig("pong-ai-trials-vs-time-graph.png")
        plt.close(fig)
    ...
    def play(self, winning_score=11, qlearn_mode=False):
        ...
        num_samples = 1000  # max number of samples we want to sample.
        sample_mod_num = 0
        prev_sim_num = -1
        ...
        pong_game_start_time = time.time()
        ...
        while self.ai1.score < winning_score and self.ai2.score < winning_score:
            if self.qlearn_mode:
                curr_time = time.time()
                trial_elapsed_time = curr_time - trial_start_time
                # Stop the q-learning if a trial exceeds the given time limit (in minutes).
                ...
                if self.ai2.score > prev_sim_num and self.ai2.score % sample_mod_num == 0:
                    total_game_time = round((curr_time - pong_game_start_time) / 60 / 60, 2) # in hours
                    self.times.append(total_game_time)
...
if __name__ == '__main__':
    ...
    if os.path.isfile('pong-qtable.dat'):
        ...
        pong_game.plot_num_trials_verses_time()
