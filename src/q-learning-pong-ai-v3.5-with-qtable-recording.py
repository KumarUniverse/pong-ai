...
class Paddle():
    ...
    def __init__(self, pong_game, side):
        ...
        self.y = (pong_game.canvas_height // 2) - (self.height // 2)
        self.score = 0
...
class PongGame(pyglet.window.Window):
    ...
    def play(self, winning_score=11, qlearn_mode=False):
        ...
        while self.ai1.score < winning_score and self.ai2.score < winning_score:
            ...
            # Get the last x records after Q agent misses the ball.
            if self.ball.x <= self.ai1.x - self.ball.width:
                self.qagent.missed_q_records.append(self.qagent.all_q_records[-self.qagent.missed_records_size:])
                self.qagent.all_q_records.clear()
                break
        ...
        print("Missed q records:")
        print(self.qagent.missed_q_records[-1])
...
class Qagent():
    ...
    def __init__(self, pong_game, paddle, opponent, ball):
        ...
        self.record_num = 1
        self.missed_records_size = 30 # Store the last 10 q table records right after paddle misses ball.
        self.all_q_records = []
        self.missed_q_records = []
    ...
    def play_game(self, ball_y, pad_y):
        ...
        best_next_action = 0
        actions_q_values = self.qtable[ball_y][pad_y]
        self.all_q_records.append(("rnum: " + str(self.record_num), "ball_y: " + \
            str(ball_y), "pad_y: " + str(pad_y), "qtable: " + \
                str(self.round_list(actions_q_values, 3))))
        self.record_num += 1
        # try:
        #     new_ball_y = ball_y + self.ball_actions[self.ball.move_y]
        #     if new_ball_y < 0:
    ...
    def round_list(self, l, num_digits):
        l2 = []
        for x in l:
            l2.append(round(x, num_digits))
