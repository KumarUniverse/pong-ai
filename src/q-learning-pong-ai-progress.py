...
class PongGame(): #pyglet.window.Window):
    ...
    def play(self, winning_score=11, qlearn_mode=False):
        ...
        # Save the Q agent's Q-table at 4 equally spaced checkpoints in the
        # agent's learning process.
        qagent_progress_checkpoints = set()
        for i in range(1,4):
	        qagent_progress_checkpoints.add(int(winning_score*(i/4)))
        q_table_i = 1 # Q table index
        ...
        while self.ai1.score < winning_score and self.ai2.score < winning_score:
            if self.qlearn_mode:
                if self.ai2.score in qagent_progress_checkpoints:
                    print(f"Hello{q_table_i}")
                    qtable_filename = f'pong-qtable-part{q_table_i}.dat'
                    self.qagent.write_q_table(qtable_filename)
                    qagent_progress_checkpoints.remove(self.ai2.score)
                    q_table_i += 1
        ...
        if qlearn_mode:
            qtable_filename = f'pong-qtable-part{q_table_i}.dat'
            self.qagent.write_q_table(qtable_filename)  # Save the Q table in a file.
...
class Qagent():
    ...
    def qlearn(self, ball_y, pad_y):
        ...
        self.epsilon = max(0.01, round(1 - (self.opponent.score+1)/self.pong_game.winning_score, 2))
...
if __name__ == '__main__':
    ...
    num_simulations = 1000 # 100000000
    if os.path.isfile('pong-qtable-part4.dat'):
        print("Game started.")
        pong_game.play()
        print("Game finished.")
    else:
