...
# 3 = INFO, WARNING, and ERROR messages are not printed. Only FATAL errors are logged.
...
class PongGame(pyglet.window.Window):
    ...
    def get_img_array(self):
        ...
        img_arr = list(np.asarray(img))[::-1] # dimension: 166 x 233
        # print(np.asarray(img).dtype) # dtype: uint8
        # x = input()
        #img_arr = np.asarray(list(np.asarray(img))[::-1]) # dimension: 166 x 233, for debugging
    ...
    def play(self, winning_score=11, qlearn_mode=False, neuralnet_filename='pong-ai-deep-q-conv-network.h5'):
        """
        ...
        while True: #self.ai1.score < winning_score and self.ai2.score < winning_score:
            ...
            if self.qlearn_mode:
                ...
                if self.ai2.score > 0 and self.ai2.score % 100 == 0:
            ...
            if self.turn:
                ...
                # At the start of each game, alternate ball initialization
                # from top and bottom.
            ...
            if self.ball.x <= self.ai1.x - self.ball.width:  # ai1 lost, ai2 won the round.
                trial_start_time = time.time()
                self.render_pong_canvas()
                pong_frame = self.get_img_array()
                reward = -100 #-1
            ...
            if self.ball.y <= self.ai1.y + self.ai1.height and \
                ...
                #reward = 1 # pseudo-reward for keeping ball within y range of paddle.
                if self.ball.x <= self.ai1.x + self.ai1.width and \
                    self.ball.x + self.ball.width >= self.ai1.x:
                    #if self.qlearn_mode:
                    ...
                    angle_factor = max(angle_factor, 0.1) # prevent angle factor from being 0.
                    ball_vector_angle = 70 * math.pi / 180 * angle_factor
                    abs_diff = abs(ball_center - pad_center)
                    # Give the Q-agent a higher reward if it hits the paddle on the paddle's edge.
                    reward_weight = ((self.ai1.height // 2) + (self.ball.height // 2)) - abs_diff
                    reward_weight = max(0, reward_weight) # no negative rewards.
                    reward = 5 * (1 / (reward_weight + 1)) # pseudo-reward for hitting the ball.
                    #reward = 5 #0.5 # pseudo-reward for hitting the ball.
                    # else:
            ...
            # Reward the agent for every action it takes.
            # If a reward is not already given, give it an edge reward,
            # where the agent is rewarded higher when the ball is within range
            # of the paddle and the difference between the ball's center and
            # paddle's center is great.
            # if reward == 0:
            #     ball_center = self.ball.y + (self.ball.height // 2)
            #     pad_center = self.ai1.y + (self.ai1.height // 2)
            #     abs_diff = abs(ball_center - pad_center)
            #     reward_weight = ((self.ai1.height // 2) + (self.ball.height // 2)) - abs_diff
            #     reward_weight = max(0, reward_weight) # no negative rewards.
            #     reward = 1 / (reward_weight + 1)

            # If a reward is not already given to the agent, give it a reward.
            # If the agent moves its paddle up or down, give it a positive reward.
            # If the agent idles its paddle, give it a negative reward.
            if reward == 0:
                if self.qagent_action == 0:
                    reward = -1
                else:
        ...
        if qlearn_mode:
            ...
            self.qagent.save_target_model_weights(neuralnet_filename) # UNCOMMENT!
            self.qagent.plot_loss('pong-ai-conv-deep-q-loss.png')
...
class Qagent():
    ...
    def __init__(self, pong_game, paddle, episode_limit):
        ...
        self.max_buffer_len = 20000 #100000
        ...
        #     ]
        # , dtype=np.uint8) # dimensions: 4x166x233
        self.state = np.asarray(
            [
                [
                    [0 for _ in range(self.state_depth)]
                    for _ in range(self.state_width)
                ]
                for _ in range(self.state_height)
            ]
        , dtype=np.uint8) # dimensions: 166x233x4
        self.state = tf.convert_to_tensor(self.state, dtype=tf.uint8)
        self.action = 0
        # Q models:
        ...
        self.alpha = 0.00025 #0.001 #0.00025 # learning rate.
    ...
    def create_q_model(self):
        ...
        # Input layer: input image is 166x233x4 pixels.
        inputs = layers.Input(shape=(self.state_height, self.state_width, self.state_depth))
        # Hidden layers: (# of filters, kernal size, stride, activation). filter size > stride
        layer1 = layers.Conv2D(16, 8, strides=4, activation="relu",
        bias_regularizer=regularizers.l2(1e-4),
        activity_regularizer=regularizers.l2(1e-5))(inputs)
        layer2 = layers.Conv2D(32, 4, strides=2, activation="relu",
        bias_regularizer=regularizers.l2(1e-4),
        activity_regularizer=regularizers.l2(1e-5))(layer1)
        layer3 = layers.MaxPooling2D(pool_size=(4,4), strides=2)(layer2)
        layer4 = layers.Flatten()(layer3) # works better with square images.
        layer5 = layers.Dense(256, activation="relu",
        kernel_regularizer=regularizers.l2(1e-4),
        bias_regularizer=regularizers.l2(1e-4),
        activity_regularizer=regularizers.l2(1e-5))(layer4)

        # Original layers:
        # # Input layer: input image is 4x166x233 pixels.
        # inputs = layers.Input(shape=(self.state_depth, self.state_height, self.state_width))
        # # Hidden layers: (# of filters, kernal size, stride, activation). filter size > stride
        # layer1 = layers.MaxPool2D(pool_size=(self.state_depth,1))(inputs)
        ...
        action_layer = layers.Dense(num_pad_actions, activation="relu")(layer5) # softmax for Huber loss
    ...
    def preprocess_frames(self):
        ...
        # state_tensor = tf.convert_to_tensor(np.asarray(self.frame_history, dtype=np.uint8), dtype=tf.uint8)
        # self.frame_history.clear()
        # return state_tensor

        k_frames = np.asarray(self.frame_history, dtype=np.uint8)
        k_frames = np.stack(k_frames, axis=-1)
        #print("k_frames:", k_frames.shape) k_frames.shape: (166, 233, 4)
        state_tensor = tf.convert_to_tensor(k_frames, dtype=tf.uint8)
        self.frame_history.clear()
        return state_tensor
    ...
    def train_q_network(self):
        ...
        state_sample = tf.convert_to_tensor(np.asarray([self.state_history[i] for i in indices], dtype=np.uint8), dtype=tf.uint8)
        #state_sample = [self.state_history[i] for i in indices]
        action_sample = [self.action_history[i] for i in indices]
        rewards_sample = [self.rewards_history[i] for i in indices]
        next_state_sample = tf.convert_to_tensor(np.asarray([self.next_state_history[i] for i in indices], dtype=np.uint8), dtype=tf.uint8)
    ...
    def q_learn(self, pong_frame, reward):
        ...
        if self.frame_count > self.max_steps_per_episode: # Episode finished.
            ...
            #self.frame_count = 0
            last_frame = True
        elif abs(reward) == 100: # Pong episode finished.
            #self.episode_count += 1
        ...
        if self.frame_count % self.update_every_kth_frame == 0:
            ...
            if rand_num < self.epsilon: # or self.state is None: # exploration
                ...
                tf.print("action probs:", action_probs, output_stream=sys.stdout)
                print(f"frame count: {self.frame_count} (exploration)")
            else:                       # exploitation
                ...
                self.action = tf.math.argmax(action_probs[0]).numpy()
                tf.print("action probs:", action_probs, output_stream=sys.stdout)
                print(f"frame count: {self.frame_count} (exploitation)")
                #print(f"self.action: {self.action}") # for debugging, printed 0, 1
        ...
        if last_frame:
            ...
            self.losses.append(abs(avg_loss))
            self.episode_count += 1
            self.frame_count = 0
            print(f"Episode count: {self.episode_count}")
    ...
    def play_game(self, pong_frame, reward):
        ...
        if abs(reward) == 100: # Pong episode finished.
             while self.frame_count % self.update_every_kth_frame != 0:
                self.frame_history.append(pong_frame)
                self.frame_count += 1
    ...
    def simple_moving_avg(self, a, window=3):
        """
    ...
    def save_target_model(self, filename): # Eg. filename: 'pong-ai-deep-q-conv-network.h5'
        """Save the Deep Q network to a HDF5 file after finishing with the training."""
        self.target_model.save(filename)

    def load_target_model(self, filename): # Eg. filename: 'pong-ai-deep-q-conv-network.h5'
...
def main():
    num_episodes = 5000
    network_filename = 'pong-ai-deep-q-conv-network.h5'
